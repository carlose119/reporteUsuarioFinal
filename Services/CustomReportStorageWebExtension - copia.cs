...
using reporte.PredefinedReports;
using reporte.Data;
...
namespace reporte.Services
{
    public class CustomReportStorageWebExtension : DevExpress.XtraReports.Web.Extensions.ReportStorageWebExtension
    {
        ...
        public override byte[] GetData(string url)
        {
            ...
            var reportData = DbContext.Reports.FirstOrDefault(x => x.Name == reportName);
            if (reportData != null) 
            {                
                // Apply the parameter values to the report.
                var parameters = HttpUtility.ParseQueryString(parametersQueryString);
                foreach (string parameterName in parameters.AllKeys) {
                    reportData.Parameters[parameterName].Value = Convert.ChangeType(
                ...
                foreach (var parameter in reportData.Parameters) {
